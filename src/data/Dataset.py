...
class Dataset:
    ...
    def __init__(self, file_extensions: tuple, extract_method=None, args=None):
        """
        Parameters
        ----------
        file_extensions : tuple
            File extensions of the data, format: ("*ext1", "ext2",...)
        extract_method : method
    ...
    def create_dataset(self, path: list) -> pd:
        ...
        for file_ext in self.file_extensions:
            paths += [y for x in os.walk(path) for y in glob(os.path.join(x[0], file_ext))]
